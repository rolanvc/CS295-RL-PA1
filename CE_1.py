...
def main():
    ...
    env = OneHotWrapper(env)
    gamma0 = 0.9
    mean_rewards0 = cross_entropy(env, gamma0)
    f, sp = plt.subplots()
    sp.plot(mean_rewards0, 'b', label='Average rewards at gamma={}'.format(gamma0))
    gamma1 = 0.5
    mean_rewards1 = cross_entropy(env, gamma1)
    sp.plot(mean_rewards1, 'r', label='Average rewards at gamma={}'.format(gamma1))

    gamma2 = 0.2
    mean_rewards2 = cross_entropy(env, gamma2)
    sp.plot(mean_rewards2, 'g', label='Average rewards at gamma={}'.format(gamma2))
